...
import pandas as pd
import numpy as np
from CPR.utils import timer, preprocessing
import time
import os
...
def preprocessing_gen_model(data_path, img_list_train):
    ...
    data_vector_all = np.asarray(data_vector_list)
    data_vector_all = data_vector_all.reshape((data_vector_all.shape[1], data_vector_all.shape[2]))
...
def lr_plots(lrRangeFinder, lr_plots_path):
    ...
    plt.savefig(lr_plots_path / 'smooth.png')
    ...
    plt.savefig(lr_plots_path / 'lrRange.png')
    plt.close('all')
