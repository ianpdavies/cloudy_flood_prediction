...
def log_reg_uncertainty(img_list, pctls, feat_list_new, data_path, batch, remove_perm=True):
    for j, img in enumerate(img_list):
        preds_path = data_path / batch / 'predictions' / img
        bin_file = preds_path / 'predictions.h5'
        metrics_path = data_path / batch / 'metrics' / 'testing' / img
        ...
        for i, pctl in enumerate(pctls):
            ...
            beta_hat = trained_model.intercept_

            del predictions, X_test, y_test, data_test, data_vector_test, data_ind_test
...
# N = X_test.shape[0]
# p = X_test.shape[1] + 1
# coefs = trained_model.coef_.reshape(X_test.shape[1], 1)
# intercept = trained_model.intercept_.reshape(1, 1)
# beta_hat = np.row_stack([intercept, coefs]).reshape(data_shape[1], )
# beta_hat = [trained_model.intercept_[0], trained_model.coef_[0:data_shape[1]]]
