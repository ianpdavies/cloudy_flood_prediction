...
# ======================================================================================================================
# Measuring dissimilarity between random images
...
#                                                                                              'trial2')
#
# np.sum(~np.isnan(data_test1[:, :, 1]))
# np.sum(~np.isnan(data_test2[:, :, 1]))
#
# np.sum(~np.isnan(data_test1[:, :, 1])) / (
#             np.sum(np.isnan(data_test1[:, :, 1])) + np.sum(~np.isnan(data_test1[:, :, 1])))
# np.sum(~np.isnan(data_test1[:, :, 1])) / (data_test1.shape[0] * data_test1.shape[1])
#
# data_test2, data_vector_test2, data_ind_test2, feat_keep_test2 = preprocessing_random_clouds(data_path, img, pctl,
#                                                                                              'trial2')
...
# ======================================================================================================================
# Just comparing the means of each variable in each trial
...
feat_list = ['GSW_maxExtent', 'GSW_distExtent', 'aspect', 'curve', 'developed', 'elevation',
                 'forest', 'hand', 'other_landcover', 'planted', 'slope', 'spi', 'twi', 'wetlands']
...
#         for pctl in pctls:
#             print(pctl)
#             data_train, data_vector_train, data_ind_train, feat_keep_train = \
...
#         for pctl in pctls:
#             print(pctl)
#             data_test, data_vector_test, data_ind_test, feat_keep_test = \
#             preprocessing_random_clouds(data_path, img, pctl, trial)
#             perm_index = feat_list_new.index('GSW_perm')
#             data_vector_train = np.delete(data_vector_test, perm_index, axis=1)  # Remove GSW_perm column
#             for feat in feat_list:
#                 index = feat_list.index(feat)
